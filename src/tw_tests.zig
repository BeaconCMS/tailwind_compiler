...
fn compile(alloc: std.mem.Allocator, candidates: []const []const u8) ![]const u8 {
    return compiler.compile(alloc, candidates, null, false, true, null, null, null);
}
...
test "tw: custom utilities  referencing custom utilities in custom uti" {
    ...
    const custom_utils =
        \\{"bar":"color:red"}
    ;
    const result = try compiler.compile(alloc, &candidates, null, false, true, null, custom_utils, null);
    defer alloc.free(result);
